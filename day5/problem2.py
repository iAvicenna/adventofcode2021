...
def generate_lines(xcoords, ycoords):
    ...
    elif abs(ycoords[1]-ycoords[0]) == abs(xcoords[1]-xcoords[0]):
        
        stepx = np.sign(xcoords[1]-xcoords[0])
        stepy = np.sign(ycoords[1]-ycoords[0])
        
        line_points = [(i, j) for i,j in zip(range(xcoords[0],xcoords[1]+stepx,stepx),range(ycoords[0],ycoords[1]+stepy,stepy))]
